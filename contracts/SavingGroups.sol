...
contract SavingGroups is Modifiers {
    ...
    address public constant devAddress =
        0x84052CEc1d08cF2eB93ffBaB096b88b455Bb9EEE;
        ...
        atStage(Stages.Setup)
    {
        ...
        if (users[removeAddress].availableCashIn > 0) {
            ...
            cUSD.transfer(users[removeAddress].userAddr, availableCashInTemp);
        }
    ...
    /*Primero se verifica si hay pagos pendientes al día y se abonan,
    si sobra se verifica si se debe algo al CashIn y se abona */
        ...
        atStage(Stages.Save)
    {
        ...
        cUSD.transfer(users[msg.sender].userAddr, savedAmountTemp);
        savedAmountTemp = 0;
    }
    ...
    function completeSavingsAndAdvanceTurn(uint8 turno)
        private
        atStage(Stages.Save)
    {
    ...
    function payLateFromSavings(address _userAddress)
        internal
        atStage(Stages.Save)
    {
    ...
    function endRound() public atStage(Stages.Save) {
        ...
        for (uint8 i = 0; i < groupSize; i++) {
            ...
            cUSD.transfer(users[userAddr].userAddr, amountTemp);
